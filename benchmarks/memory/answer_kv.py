...
from inferscale.v1.kv.chunk_store import build_chunk_store
from inferscale.v1.kv.encoder import ChunkedRopeEncoder, load_encoder_tokenizer
from inferscale.v1.kv.prompt import build_prompt_tokens, require_identical_token_ids
...
from inferscale.v1.serving.vllm import (
    VLLMEngine,
...
class KVAnswerClient:
    ...
    def __init__(self, config: MemoryRunConfig) -> None:
        ...
        # store. kv_store_backend selects where the fact-chunk corpus lives
        # (the per-sample chunk store).
    ...
    def precompute_sample_cache(
        ...
        composer: SampleComposer | None = None
        cache_hit = False
        cache_path = None
        payload_meta = None
        ...
        if self.config.kv_chunk_cache_enabled:
            ...
            if cached is not None:
                ...
                if scaffold_chunks_match(cached.scaffold_chunks, scaffold):
                    composer = SampleComposer.from_cached(
    ...
    def answer_with_retrieved_memory(
        ...
        *,
        ...
        if not self._engine.started or self._tokenizer is None:
            raise RuntimeError("KVAnswerClient.prepare_sample() must be called before answering.")
        ...
        require_identical_token_ids(composed.token_ids, live_memory.token_ids)
