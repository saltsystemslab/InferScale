...
from inferscale.v1.types import EncodingPlan, KVChunk, SearchHit
...
def build_fact_context_encoding_plan(
    ...
    *,
    ...
    """Plan one fact's encode with its preceding conversation turns as the prefix."""
    if context_window < 0:
        raise ValueError("context_window must be >= 0.")
    ...
    return build_encoding_plan(
        ...
        context_ids=tuple(turn.id for turn in context_turns),
...
def build_memory_fact_plan(
    ...
    *,
    ...
    fact_token_ids: Mapping[str, list[int]],
    encoded_chunks: Mapping[str, KVChunk] | None = None,
    ...
    if encoded_chunks is not None:
        ...
        for fact in selected_facts:
            ...
            plan_context_turn_ids.extend(chunk.context_ids)
