...
from benchmarks.common.cache_identity import (
    atomic_write_json,
    endpoint_cache_key,
    normalize_endpoint,
    safe_path_part,
...
from benchmarks.common.vector_types import SearchHit, VECTOR_DISTANCE
...
class FactCatalogStore:
    ...
    def load(self, sample: ConversationSample) -> tuple[MemoryFact, ...]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Missing Mem0 fact catalog at {path}. Rerun the extract-facts stage "
                "with the same dataset, memory LLM model, endpoint, and cache dir."
            ) from exc
        except (OSError, json.JSONDecodeError) as exc:
            raise RuntimeError(
                f"Corrupt Mem0 fact catalog at {path}. Rerun the extract-facts stage "
                "to replace it."
            ) from exc
