...
Driven by the precompute-kv stage (one model and context window per
invocation). Requires the Mem0 fact catalogs to exist already (the
extract-facts stage); needs no vLLM server since the encoder runs in-process.
"""
        ...
        for sample in samples:
            ...
            if not catalog_path.exists():
                raise RuntimeError(
                    ...
                    "the extract-facts stage for this model."
                )
