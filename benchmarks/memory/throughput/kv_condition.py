...
from benchmarks.memory.chunk_cache import (
    CachedSampleEncode,
    cache_meta,
    cache_path_for,
    load_sample_chunks,
...
from benchmarks.memory.throughput.projection import check_kv_gpu_projection
from benchmarks.memory.throughput.reporting import build_result_row
from benchmarks.memory.throughput.stores import (
    build_user_store,
    close_mem0,
    fact_catalog_store,
    load_samples,
...
from benchmarks.memory.throughput.workload import (
    LocomoRequest,
...
def run_kv_injection(config: ThroughputConfig, num_users: int) -> dict[str, Any]:
    ...
    from inferscale.v1.kv.encoder import (
        TOKENIZER_PARITY_PROBE_TEXT,
        ChunkedRopeEncoder,
        load_encoder_tokenizer,
        require_tokenizer_parity,
    )
    from inferscale.v1.kv.registry import (
        drop_namespace,
        get_gpu_memory_store,
        namespace_stats,
        register_user_memory,
    )
    from inferscale.v1.serving.vllm import build_kv_transfer_config, force_vllm_inprocess_mode
    ...
    try:
        ...
        if all(sample.sample_id in cached_by_sample for sample in used_samples):
            ...
            header_chunk = encoder.encode("scaffold:header", scaffold.header_token_ids)
            footer_chunk = encoder.encode("scaffold:footer", scaffold.footer_token_ids)
            # The condition composes with header/footer only, but the cache
            # payload is shared with the accuracy composer, which also needs
            # the empty-memory chunk.
            empty_memory_chunk = encoder.encode("scaffold:empty_memory", scaffold.empty_token_ids)
        ...
        for sample_number, sample in enumerate(used_samples, start=1):
            ...
            if cached is not None:
                ...
                for fact in kv_facts:
                    ...
                    fact_chunks[fact.memory_id] = encoder.encode_plan(plan)
                if config.kv_chunk_cache_enabled:
                    cache_path, payload_meta = sample_cache_key(sample)
                    try:
                        save_sample_chunks(
            ...
            chunks_by_sample[sample.sample_id] = register_chunks(chunk_store, fact_chunks)
        encoder_probe_token_ids = encode_text_no_special(
            tokenizer, TOKENIZER_PARITY_PROBE_TEXT
        )
        ...
        for user_index in range(num_users):
            ...
            for request in requests_by_user[user_index]:
                ...
                if selected_ids:
                    release_chunks(chunk_store, selected_ids)
        ...
        del header_chunk, footer_chunk, empty_memory_chunk
        torch.cuda.empty_cache()

        transfer_config = build_kv_transfer_config(
        ...
        require_tokenizer_parity(encoder_probe_token_ids, llm.get_tokenizer())
