...
from benchmarks.common.stats import mean, numeric_summary, safe_div
...
def summarize_rag_records(
    ...
    *,
    ...
    for key in ANSWER_METRIC_KEYS:
        metrics[key] = mean(_answer_metric_values(rows, key))
    metrics["answer_metrics_by_type"] = {
        question_type: {
            key: mean(_answer_metric_values(type_rows, key)) for key in ANSWER_METRIC_KEYS
        }
    ...
    metrics["retrieval"] = {
        ...
        **{
            key: mean(_retrieval_metric_values(retrieval_rows, key))
            for key in RETRIEVAL_METRIC_KEYS
        },
    }
    for key in TIMING_METRIC_KEYS:
        summary = numeric_summary(_timing_metric_values(rows, key))
        if summary["count"]:
            metrics[key] = summary
