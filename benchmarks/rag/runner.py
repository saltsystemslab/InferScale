...
from benchmarks.common.files import JsonlWriter, write_json
from benchmarks.common.files import read_json_or_default, read_jsonl, replace_jsonl
from benchmarks.common.system import collect_system_metadata

from benchmarks.rag.chunking import chunk_corpus, corpus_fingerprint
from benchmarks.rag.config import RagBenchConfig
from benchmarks.rag.data_types import RagChunk, RagDocument, RagQuery
...
from benchmarks.common.judge import failed_judge_payload, format_accuracy, is_judged, skipped_judge_payload
...
from benchmarks.rag.kv_cache import cache_meta_base, missing_chunk_files, rag_chunk_cache_dir
...
def run_estimate(config: RagBenchConfig) -> dict[str, Any]:
    ...
    # Machine-readable line consumed by the precompute-kv stage's disk-space check.
    print(
        "ESTIMATE "
...
def run_answer(config: RagBenchConfig) -> dict[str, Any]:
    ...
    if config.answer_backend == "kv-injection":
        ...
        if missing:
            raise RuntimeError(
                ...
                f"{cache_dir} (first missing: {missing[0]}). Run the precompute-kv stage for "
                f"dataset {config.dataset_name}, model {config.model}, chunk size "
                f"{config.chunk_size}, context window {config.context_window}."
            )
    ...
    from benchmarks.rag.embedder import build_cached_embedder
    from benchmarks.rag.retrieval import CorpusRetriever
...
def judge_existing_run(config: RagBenchConfig) -> dict[str, Any]:
    ...
    if judge_client is None:
        raise RuntimeError("Deferred judging requires the vllm judge provider.")
    ...
    for row_number, record in enumerate(records, start=1):
        ...
        try:
            ...
            record["judge"] = failed_judge_payload(exc, judge_metadata(config))
            _write_rag_outputs(
                config,
                predictions_path,
                records,
            ...
            raise RuntimeError(
                ...
                f"rerun the judge stage. Original error: {type(exc).__name__}: {exc}"
            ) from exc
