...
# RAG per-chunk disk cache, per (model, window, chunk size). Idempotent and
# resumable: existing chunk files are skipped.
#
...
#   PRECOMPUTE_MODELS="llama" RAG_WINDOWS="5" RAG_CHUNK_SIZES="1024" bash scripts/rag/precompute_kv.sh
...
PRECOMPUTE_MODELS="${PRECOMPUTE_MODELS:-llama}"
...
for MODEL_ALIAS in ${PRECOMPUTE_MODELS}; do
  for W in ${RAG_WINDOWS}; do
    for C in ${RAG_CHUNK_SIZES}; do
      echo "=== Estimating RAG KV cache: model=${MODEL_ALIAS} window=${W} chunk_size=${C} ==="
      ...
      echo "=== Precomputing RAG KV chunks: model=${MODEL_ALIAS} window=${W} chunk_size=${C} ==="
      rag-jasper-bench \
        --dataset-name "${DATASET_NAME}" \
        --answer-model "${MODEL_ALIAS}" \
        --chunk-size "${C}" \
...
echo "=== RAG KV precompute complete for: ${PRECOMPUTE_MODELS} (windows: ${RAG_WINDOWS}, chunk sizes: ${RAG_CHUNK_SIZES}) ==="
