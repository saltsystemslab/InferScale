...
case "${ROLE}" in
  answer)
    ...
    ;;
  judge)
    MODEL="${JUDGE_VLLM_MODEL:-google/gemma-3-12b-it}"
    API_KEY="${JUDGE_VLLM_API_KEY:-${JUDGE_API_KEY:-token-abc123}}"
    HOST="${JUDGE_VLLM_HOST:-${VLLM_HOST:-127.0.0.1}}"
    PORT="${JUDGE_VLLM_PORT:-8001}"
    SERVED_MODEL_NAME="${JUDGE_VLLM_SERVED_MODEL_NAME:-${JUDGE_MODEL:-${MODEL}}}"
    TP="${JUDGE_VLLM_TP:-${VLLM_TP:-1}}"
    GPU_MEMORY_UTILIZATION="${JUDGE_VLLM_GPU_MEMORY_UTILIZATION:-${VLLM_GPU_MEMORY_UTILIZATION:-0.80}}"
    MAX_MODEL_LEN="${JUDGE_VLLM_MAX_MODEL_LEN:-${VLLM_MAX_MODEL_LEN:-32768}}"
    DTYPE="${JUDGE_VLLM_DTYPE:-${VLLM_DTYPE:-auto}}"
    QUANTIZATION="${JUDGE_VLLM_QUANTIZATION:-${VLLM_QUANTIZATION:-}}"
    ;;
  *)
    echo "error: VLLM_ROLE must be 'answer' or 'judge', got '${ROLE}'." >&2
    exit 2
    ;;
esac
...
unset JUDGE_VLLM_MODEL JUDGE_VLLM_API_KEY JUDGE_VLLM_TP JUDGE_VLLM_GPU_MEMORY_UTILIZATION
unset JUDGE_VLLM_MAX_MODEL_LEN JUDGE_VLLM_DTYPE JUDGE_VLLM_QUANTIZATION
