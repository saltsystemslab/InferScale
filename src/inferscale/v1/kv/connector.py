...
from .registry import get_gpu_memory_store
...
# Strict GPU mode runs the vLLM engine in-process, so the driver process can
...
# which is what a fail-fast coverage check needs to be exact.
...
class MemoryKVConnector(KVConnectorBase_V1):
    """Strict GPU-only KV connector for composed memory tensors."""
    ...
    def __init__(
        ...
        if memory_path is not None:
            raise RuntimeError(
                ...
                "Register GPU tensors through inferscale.v1.kv.registry."
        )
        ...
        if self._allow_prefix_scan:
            logger.warning(
                ...
                "This is unsafe for measurements unless explicitly intended."
            )
        logger.info(
