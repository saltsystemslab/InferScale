...
def _percentile(sorted_numbers: list[float], fraction: float) -> float:
...
def _pct(sorted_values: list[float], fraction: float) -> float:
    return _percentile(sorted_values, fraction) if sorted_values else 0.0
    ...
    def get_user_memory(self, user_id: str) -> UserMemory | None:
        ...
        if slot is None:
            raise RuntimeError(
                ...
                "increase the staging slot count."
            )
        return UserMemory(
