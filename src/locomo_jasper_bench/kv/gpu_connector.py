...
class MemoryKVConnector(KVConnectorBase_V1):
    ...
    def get_num_new_matched_tokens(
        ...
        if num_computed_tokens > 0:
            raise RuntimeError(
                ...
                f"(num_computed_tokens={num_computed_tokens}); block bookkeeping would inject wrong positions. "
                "Keep enable_prefix_caching=False for the answer engine."
            )
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs: Any) -> None:
        ...
        for load in metadata.loads:
            ...
            if memory is None:
                raise RuntimeError(f"Strict GPU memory for user {load.user_id} was not found during load.")

            first_tensor = next(iter(memory.kv_by_layer.values()), None)
            if first_tensor is None:
                raise RuntimeError(f"Strict GPU memory for user {load.user_id} has no layer tensors.")
