...
from ..runtime_paths import default_embedding_cache_dir, default_results_root
...
# the retrieved turns' KV instead of their text.
...
class ThroughputConfig:
    ...
    def to_jsonable(self, *, redact_secrets: bool = True) -> dict[str, Any]:
        data = asdict(self)
        for key in ("results_dir", "dataset_path", "embedding_cache_dir"):
...
def parse_args(argv: list[str] | None = None) -> tuple[ThroughputConfig, bool]:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Run multi-user throughput benchmarks for vLLM memory conditions over "
            "the LoCoMo dataset's raw conversation turns."
        ),
        allow_abbrev=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Number of previous LoCoMo sessions used as an encoding-only prefix when "
            "pre-RoPE encoding each memory turn in the kv_injection condition. "
            "0 encodes each turn in isolation."
        ),
    )
    parser.add_argument(
