...
from .config import ALL_CONDITIONS, BenchmarkPoint, ThroughputConfig, matrix_text
            ...
            for point in config.matrix:
                specs.append(
                    WorkerSpec(
                        condition=condition,
                        matrix=(point,),
                        output_path=worker_dir / f"{condition}-{point.num_users}u-{point.memory_tokens}t.json",
                    )
                )
        else:
            specs.append(
...
def run_throughput(config: ThroughputConfig, *, dry_run: bool = False) -> dict[str, Any] | None:
    ...
    if dry_run:
        ...
        print(f"matrix points: {len(config.matrix)}")
        for spec in specs:
    ...
    for index, spec in enumerate(specs, start=1):
        ...
        print(
            f"[{index}/{len(specs)}] {spec.condition} matrix={matrix_text(spec.matrix)}",
            flush=True,
        )
...
def _load_worker_results(spec: WorkerSpec) -> list[dict[str, Any]]:
    ...
    expected = {(point.num_users, point.memory_tokens) for point in spec.matrix}
    actual = {(int(row["num_users"]), int(row["memory_tokens"])) for row in rows}
    if actual != expected:
        raise RuntimeError(
            f"Worker output matrix mismatch for {spec.condition}: expected={sorted(expected)} actual={sorted(actual)}"
        )
    return rows
...
def _validate_runtime_requirements(config: ThroughputConfig) -> None:
    ...
    if "kv_injection" in config.conditions:
        required_modules.update(("torch", "transformers"))
    if "mem0" in config.conditions:
        required_modules.add("mem0")
        required_modules.add("qdrant_client" if config.vector_backend == "qdrant" else "jasper")
    ...
    if "mem0" in config.conditions and not config.embedding_api_key and not config.embedding_base_url:
        raise RuntimeError(
            "The mem0 condition requires --embedding-api-key/OPENAI_API_KEY or "
