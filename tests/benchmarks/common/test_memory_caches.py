...
from benchmarks.memory.protocol import (
...
from benchmarks.memory.mem0.llm_cache import CachedMemoryLLM, CachedMemoryLLMMissingError
from benchmarks.memory.mem0.llm_protocol import (
...
def test_cached_embedder_batch_read_mode_never_calls_wrapped_embedder(tmp_path: Path) -> None:
    ...
    with pytest.raises(CachedEmbeddingMissingError, match="preembed stage"):
...
def test_cached_embedder_concurrent_same_key_writes_are_atomic(
    ...
    monkeypatch.setattr("benchmarks.common.embedding_cache.np.save", synchronized_save)
...
def test_cached_memory_llm_read_mode_fails_on_missing_or_corrupt_entries_without_fallback(
    ...
    with pytest.raises(CachedMemoryLLMMissingError, match="extract-facts stage"):
    ...
    with pytest.raises(CachedMemoryLLMMissingError, match="corrupt.*extract-facts stage"):
        reader.generate_response(messages, response_format={"type": "json_object"})
    assert wrapped.calls == []
