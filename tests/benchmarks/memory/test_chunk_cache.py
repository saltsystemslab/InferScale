...
from benchmarks.memory.types import MemoryFact
...
def _scaffold(
    ...
    return SimpleNamespace(
        ...
        empty_token_ids=empty_memory or [9],
    )


def _chunk(token_ids: list[int]) -> KVChunk:
    return KVChunk(chunk_id="chunk", token_ids=token_ids, kv_by_layer={"layer": SimpleNamespace(nbytes=8)})
...
def test_scaffold_chunks_match_compares_all_slots() -> None:
    ...
    assert not scaffold_chunks_match({**chunks, "memory_list_header": _chunk([7])}, _scaffold([1, 2], [3]))
    assert not scaffold_chunks_match({**chunks, "footer": None}, _scaffold([1, 2], [3]))


def test_save_and_load_round_trip(tmp_path: Path) -> None:
    torch = pytest.importorskip("torch")
    from benchmarks.memory.chunk_cache import load_sample_chunks, save_sample_chunks
    ...
    def chunk(seed: int) -> KVChunk:
        return KVChunk(
            chunk_id=f"chunk-{seed}",
    ...
    assert loaded.fact_chunks["a"].context_ids == ("turn-1",)
    assert torch.equal(
