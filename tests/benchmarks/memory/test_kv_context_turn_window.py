...
from inferscale.v1.types import KVChunk, EncodingPlan
from inferscale.v1.serving.vllm import vllm_engine_kwargs
...
def test_window_selects_immediately_preceding_turns_across_sessions() -> None:
    ...
    assert plan.context_ids == (sample.turns[0].id, sample.turns[1].id)
...
class _FakeChunkEncoder:
    ...
    def __init__(self) -> None:
        ...
        self.fact_plans: list[EncodingPlan] = []
    ...
    def encode(self, chunk_id: str, token_ids: list[int]) -> KVChunk:
        ...
        return KVChunk(chunk_id=chunk_id, token_ids=list(token_ids), kv_by_layer={})
...
def test_w_greater_than_zero_discards_prefix_tokens_from_visible_chunk(
    ...
    target_plan = next(plan for plan in encoder.fact_plans if plan.chunk_id == "target")
...
def test_fact_metrics_report_selected_context_encoding() -> None:
    ...
    chunk = KVChunk(
        ...
        token_ids=[30],
        kv_by_layer={},
        context_ids=("sample-1:session_1:0", "sample-1:session_2:0"),
...
def test_live_vllm_memory_tokens_must_match_precomputed_hf_tokens() -> None:
    ...
    with pytest.raises(RuntimeError, match="differ from the live tokenizer at index=1"):
...
def test_scaffold_footer_covers_kv_tail_despite_newline_merging() -> None:
    ...
    scaffold = extract_memory_scaffold_token_ids(_NewlineMergingTokenizer(), block_size=block_size
    )
...
def test_scaffold_footer_covers_kv_tail_for_char_level_tokenizer() -> None:
    ...
    scaffold = extract_memory_scaffold_token_ids(_DeterministicTokenizer(), block_size=block_size
    )
...
def test_scaffold_footer_pad_survives_content_trimming_template() -> None:
    ...
    block_size = 16
    scaffold = extract_memory_scaffold_token_ids(_TrimmingTemplateTokenizer(), block_size=block_size
    )
