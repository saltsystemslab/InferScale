...
class _FakeStore:
    def __init__(self, *args: object) -> None:
        self.root = args[0] if len(args) > 1 else None
        self.config = args[-1]
        self.vector_count = 0
        self.dim = None
...
def test_adapter_dispatches_exhaustively(monkeypatch: pytest.MonkeyPatch, tmp_path: object) -> None:
    monkeypatch.setattr("benchmarks.memory.mem0.adapter.JasperIndex", _FakeStore)
    monkeypatch.setattr("benchmarks.memory.mem0.adapter.QdrantVectorStore", _FakeStore)

    jasper = Mem0JasperVectorStore(path=tmp_path, backend="jasper")
    qdrant = Mem0JasperVectorStore(path=tmp_path, backend="qdrant")

    assert isinstance(jasper.store.config, JasperIndexConfig)
...
def test_backend_validation_rejects_concrete_store_mismatch() -> None:
    ...
    vector_store = SimpleNamespace(
        ...
        store=object.__new__(JasperIndex),
...
def test_jasper_stages_exact_search_and_builds_graph_only_once(
    ...
    store = JasperIndex(JasperIndexConfig(beam_width=1))
    store.add_many(
        [[1.0, 0.0], [0.0, 1.0]],
...
def test_jasper_device_search_keeps_result_ids_on_device(
    ...
    monkeypatch.setattr(
        "inferscale.v1.index.jasper._device_result_is_complete",
