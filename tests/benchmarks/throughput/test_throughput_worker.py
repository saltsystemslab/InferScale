...
def test_pinned_host_projection_rejects_footprint_above_available_ram(
    ...
    monkeypatch.setattr(
        "benchmarks.memory.throughput.projection.available_host_memory_bytes",
        lambda: 10 * 2**30,
    )
...
def test_kv_warmup_prompt_extends_memory_with_random_tail() -> None:
    from benchmarks.memory.throughput.engine import build_kv_warmup_prompt
...
def test_warmup_zero_batches_still_resets_prefix_cache() -> None:
    from benchmarks.memory.throughput.engine import warm_up
...
def test_warmup_missing_reset_api_raises() -> None:
    from benchmarks.memory.throughput.engine import reset_prefix_cache
...
def test_routed_sampling_params_carry_memory_user_ids() -> None:
    from inferscale.v1.kv.connector_utils import MEMORY_USER_ID_EXTRA_ARG
    from benchmarks.memory.throughput.engine import sampling_params_with_memory_user_ids
...
def test_extract_user_id_reads_sampling_extra_args() -> None:
    ...
    from inferscale.v1.kv.connector_utils import MEMORY_USER_ID_EXTRA_ARG, extract_user_id
