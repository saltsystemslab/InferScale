...
def test_tokenizer_parity_passes_for_identical_tokenizers() -> None:
    probe_ids = encode_text_no_special(StubTokenizer(), TOKENIZER_PARITY_PROBE_TEXT)
    require_tokenizer_parity(probe_ids, StubTokenizer())


def test_tokenizer_parity_raises_for_diverging_tokenizers() -> None:
    probe_ids = encode_text_no_special(StubTokenizer(), TOKENIZER_PARITY_PROBE_TEXT)
    with pytest.raises(RuntimeError, match="tokenizers disagree"):
        require_tokenizer_parity(probe_ids, StubTokenizer(offset=1))
...
def test_encoder_loader_matches_engine_loader(model: str) -> None:
    ...
    probe_ids = encode_text_no_special(tokenizer, TOKENIZER_PARITY_PROBE_TEXT)
    require_tokenizer_parity(probe_ids, tokenizer)
